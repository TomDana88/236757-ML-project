    ...
    for id, sleep_data in unique_dict_ids.items():
        ...
        for state in ['light_sleep', 'awake_sleep', 'deep_sleep']:
            ...
            for _, row in df.iterrows():
                start_time = pd.Timestamp('2023-01-01') + pd.Timedelta(seconds=row['startTimeInSeconds'])
                end_time = pd.Timestamp('2023-01-01') + pd.Timedelta(seconds=row['endTimeInSeconds'])
    ...
    def process_sleep_df(df, sleep_state, unified_df):
        for index, row in df.iterrows():
            ...
            for minute in range(start_minute, end_minute):
                ...
                unified_df = unified_df.append({'Date': str(date).split(" ")[0], 'Hour': date.hour, 'Minute': minute_of_hour, 'SleepState': sleep_state}, ignore_index=True)
    ...
    for date in unique_dates:
        for hour in range(24):
            for minute in range(60):
                if not ((unified_sleep['Date'] == date) & (unified_sleep['Hour'] == hour) & (unified_sleep['Minute'] == minute)).any():
                    unified_sleep = unified_sleep.append({'Date': date, 'Hour': hour, 'Minute': minute, 'SleepState': '?'}, ignore_index=True)
...
def preprocess_data(data):
    ...
    for id in unique_ids:
        ...
        unified_sleep = unify_sleep_data(unique_dict_ids[id]['light_sleep'],unique_dict_ids[id]['awake_sleep'],unique_dict_ids[id]['deep_sleep'])
